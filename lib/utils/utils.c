...
//----------------------------------------------------
char *getDiaFecha(char *fechaVal)
{
     char *ffecha;
     time_t *tt;
     time_t tmm;
     ...
     if(fechaVal!=NULL)
     {      
         CLEAR(fecha, MAXFECHA);
        strcpy(fecha,fechaVal);
         
         char **rFecha;
         rFecha = fStrSplit(fecha,"-");
         anio = atoi(rFecha[0]);
         mes = atoi(rFecha[1]);
         dia = atoi(rFecha[2]);
         
         // split de la fecha para sacar los datos de dia mes y anio
          t.tm_year = anio - 1900;      
         t.tm_mon = mes - 1; 
         t.tm_mday = dia; 
         t.tm_hour = 0; 
         ...
         t.tm_isdst = -1; 
         mktime(&t);
         ffecha = malloc(sizeof(char)*MAX);
         snprintf( ffecha, MAX, "%s",dias[t.tm_wday]);
     }
     else
     {    
        tmm = time(NULL);
         struct tm tm = *localtime(&tmm);
         ffecha = malloc(sizeof(char)*MAX);
         snprintf( ffecha, MAX, "%s",dias[tm.tm_wday]);
    }     
     return ffecha;
}
char *getFechaHora()
